...
def gen_md_replica(cat_sys, out_file, in_file, xyz_path, istep, natoms, matoms, bx, by, bz):
   ...
   #extract coordinates for graphene sheets with cation==> does not change for all replicas
    with open(os.path.join(f"/home/shehani/DOE_project/int_xyz/{cat_sys}_nmeth_single_cat_box1.xyz"), 'r') as sys:
...
for i in range(st,end+1,1):
        ...
        gen_md_infiles(f"therm{i}/geo_end", f"md{i}/dftb_in", f"therm{i}", 922, 575, 30.396, 13.392, 18)
####################################################################change parameters##################################################################################################
                #os.makedirs(f"therm{i}")
        #gen_md_replica('c1', '/home/shehani/DOE_project/frozen_cat/single_cat/200sim/c1_48_h2o/md1/geo_end', f"therm{i}/dftb_in", f"therm{i}",  i*100, 449, 288, 15.198, 13.392, 15)

        with open(os.path.join(f"md{i}/sub_dftb.sh"), 'w') as sub:
                sub.write("#!/bin/sh" +  "\n")
####################################################################change job name##################################################################################################
                sub.write(f"#SBATCH --job-name=c4c4.{i}" + "\n")
...
####################################################################change numer of water and steps ##################################################################################################
                anal.write("md_analysis(98, 45000, 50000)")


        with open(os.path.join(f"md{i}/python.sh"), 'w') as py1:
                with open(os.path.join("python.sh"), 'r') as or_py1:
                        lines = or_py1.readlines()
                        for line in lines:
                                py1.write(line)

        with open(os.path.join(f"md{i}/py_therm.sh"), 'w') as py2:
        ...
        with open(os.path.join("sub_therm_multi.sh"), 'w') as therm_multi:
                ...
                therm_multi.write(f"cd md{st}/" + "\n")
                therm_multi.write("sbatch py_therm.sh &" + "\n"+ "\n")
                for j in range(st+1, end+1,1):
                        therm_multi.write(f"cd ../md{j}/"+ "\n")
                        therm_multi.write("sbatch py_therm.sh &" + "\n" + "\n")
